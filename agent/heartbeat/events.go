...
func HandleEvents(odooURL, apiKey string, events []Event) error {
	// TODO: Implement
	// marshal action payloads
	// call actions by type from actions package
	// trigger callback request to update status of event in odoo
...
func HandleEvent(event Event) (string, error) {
	...
	case "deploy":
		...
		json.Unmarshal([]byte(event.Parameters), &params)
		params.Validate()
