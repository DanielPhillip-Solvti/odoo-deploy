...
class AgentEvent(models.Model):
    ...
    agent_id = fields.Many2one('deploy.agent', string='Virtual Machine', required=True)
    ...
    parameters = fields.Json(string='Payload')
    status = fields.Selection(selection=[('pending', 'Pending'), ('success', 'Success'), ('fail', 'Fail')], default='pending')
    ...
    def _prune_old_events(self):
        self.search([
            ('timestamp', '<', fields.Datetime.to_string(fields.Datetime.now() - timedelta(minutes=EVENT_AGE_LIMIT_MINUTES))),
