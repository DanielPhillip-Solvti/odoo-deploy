...
_ENDPOINTS = {
    ...
    "restart_odoo": {"route": "/restart", "method": "POST", "required_fields": ["branch"]},
...
class AgentService:
    ...
    def reset_branch(self, environment_record):
        token = self._get_active_token(environment_record.env)
        return self._request(
            environment_record.url, "reset_branch", branch=environment_record.repository_branch, github_token=token
        )
